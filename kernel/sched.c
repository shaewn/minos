...
inline static bool vruntime_before(time_t a, time_t b) {
    ...
    // close means a = b + c for some 0 <= c <= UINT64_MAX / 2 OR b = a + c for some 0 <= c <= UINT64_MAX / 2
    // cases to consider to understand:
    // 0) if a == b, obviously a is not before b.

    // 1) when a < b, a - b is very large when a and b are close and small when a and b are not close.
    // proof:
    // assume a < b.
    ...
    // 3) if a and b are not close, and a < b, then 2^64 - 1 and b are close, and 2^63 and b are close.
    // this proposition justifies the reasoning behind our return values in cases a-d.
    // proof:
    ...
    // end proof.
    // from here, if we observe that when a >= 2^63, b - a <= 2^63 - 1, since b <= 2^64 - 1, so in this case a and b are close.
    // so a < 2^63. now, suppose a < k <= 2^63. we want to show that (a - k) mod 2^64 is close to b.
    // x := (a - k) mod 2^64 = 2^64 + (a - k) = 2^64 - k + a.
    // since k <= 2^63, -k + a >= -2^63 + a. Hence x >= 2^63 + a.
    // also, k > a, so -k + a < 0. Hence x < 2^64, or x <= 2^64 - 1.
    // if x == b, x and b are trivially close.
    // if x < b, b = x + (b - x), and 0 < b - x <= b - 2^63 - a <= 2^63 - 1 - a <= 2^63. Hence, b and x are close.
    // if b < x, then x = b + (x - b), and 0 < x - b <= x - 2^63 <= 2^63 - 1 <= 2^63. Hence, b and x are close.
    // NOTE:
    // this means that if a < b and a is not close to b, for any reasonable previous difference k that would cause a previous value x to wrap to a,
    // x will be close to b. furthermore, when we aren't dealing with wrapping, provided that we maintain a small enough maximum vruntime difference,
    // the values will always be close. in essence, a and b not being close serves as an indicator of when we have wrapped.
    // as long as we maintain a difference in active vruntimes of no more than 2^63, we will correctly handle wrapping vruntimes.
    // finally, this justifies cases c-d listed below.
    ...
    // 3 proves that as long as we maintain small enough (at most 2^63) maximum vruntime differences,
    // a and b not being close serves as an indicator that wrapping has occurred, associating the informal reasoning of ci and di with c and d.

    // a) a and b are close, and a < b (should return true).
    // b) a and b are close, and a > b (should return false).
    // c) a and b are not close, and a < b (should return false ... a must have wrapped around because we're so far apart)
    // ci) when we have wrapped, and a < b, a is the one that wrapped. in this case, we should return false, because a is not actually before b,
    // despite being numerically less.
    // d) a and b are not close, and a > b (should return true ... b must have wrapped around because we're so far apart)
    // di) when we have wrapped, and b < a, b is the one that wrapped. in this case, we should return true, because b is not actually before b,
    // despite being numerically less.
...
struct task *sched_run(void) {
    struct rb_node *node = get_leftmost_node();
    struct task *selected = node ? CONTAINER_OF(CONTAINER_OF(node, struct sched_node, run_queue_node), struct task, sched_node) : NULL;
...
void sched_ready_task(struct task *task) {
    if (task->state == TASK_STATE_TERMINATED) return;
...
bool can_preempt(void) {
    return current_task->preempt_counter == 0;
}
